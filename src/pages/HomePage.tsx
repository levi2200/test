...
export default function HomePage() {
  ...
  const generate = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      let result = styleDef.template(topic.trim(), niche, format);
  ...
  function handleRegenerate() {
    ...
    setTimeout(() => {
      ...
      // Append the new cinema overrides as a note block at the end
      const cinemaBlock = [
        `\n\n— REGENERATED VARIATION #${regenCount + 1} —`,
        `Expression override: ${newCinema.expression.label} — ${newCinema.expression.prompt}`,
        `Camera override: ${newCinema.camera.label} — ${newCinema.camera.prompt}`,
        `Text environment override: ${newCinema.textEnv.label} — ${newCinema.textEnv.prompt}`,
        `Color grade override: ${newCinema.colorGrade.label} — ${newCinema.colorGrade.prompt}`,
      ].join('\n');
