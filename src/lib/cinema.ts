...
export const EXPRESSIONS: ExpressionDef[] = [
  ...
  },
  {
    ...
    triggers: /\bnew\b|launch|\bjust\b|announ|\bgot\b|made|got paid|earned|big news|update|release|drop/i,
  },
  {
    id: 'calculated_cold',
...
export const COLOR_GRADES: ColorGradeDef[] = [
  ...
  },
  {
    ...
    triggers: /tech|code|\bai\b|robot|system|algorithm|data|cold|calculated|serious|professional/i,
  },
  {
    id: 'blood_red_black',
...
export const BACKGROUNDS: BackgroundDef[] = [
  ...
  // ── MONEY & FINANCE ─────────────────────────────────────────────────────────
  {
    ...
    fitStyles: ['revenue', 'confession'],
    fitNiches: ['finance', 'lifestyle'],
  ...
  },
  {
    ...
    fitStyles: ['revenue', 'mrbeast'],
    fitNiches: ['travel', 'lifestyle', 'finance'],
    triggers: /private jet|airplane|aircraft|plane|airport|jet|fly|travel rich|millionaire|billionaire|success|wealthy/i,
  },
  {
    id: 'luxury_car_interior',
    label: 'Supercar Interior',
    prompt: 'luxury supercar interior — hand-stitched leather dashboard, carbon fiber accents, digital instrument cluster glowing with performance data, paddle shifters behind a racing-inspired steering wheel, racing bucket seats with high bolsters, the blurred city lights streak past the side windows, atmosphere of peak engineering and speed',
    fitStyles: ['mrbeast', 'revenue'],
    fitNiches: ['lifestyle', 'finance'],
    triggers: /car|supercar|driving|\btaxi\b|lamborghini|ferrari|porsche|tesla|bugatti|road|highway|speed|drive/i,
  },
  {
    id: 'luxury_yacht_deck',
    label: 'Luxury Yacht Deck',
    prompt: 'luxury yacht teak wood deck at sunset — polished stainless steel railings reflecting the golden sky, comfortable white outdoor lounge furniture, the wake of the boat creating a white foam trail on the deep blue ocean water, a glimpse of a glass of champagne on a side table, atmosphere of ultimate freedom and maritime wealth',
    fitStyles: ['mrbeast', 'revenue'],
    fitNiches: ['travel', 'lifestyle', 'finance'],
    triggers: /boat|yacht|sailing|ship|deck|cruise|maritime|sea|ocean|water/i,
  },
  {
    id: 'luxury_hotel_suite',
    label: 'Luxury Hotel Suite',
    prompt: 'ultra-luxury 5-star hotel suite — floor-to-ceiling windows with sheer white curtains, designer furniture in neutral tones, a large plush king-size bed with perfectly crisp white linens in the background, a bottle of mineral water and orchid on a marble table, expensive ambient lighting, atmosphere of pure comfort and high-end hospitality',
    fitStyles: ['revenue', 'mrbeast', 'beforeafter'],
    fitNiches: ['travel', 'lifestyle'],
    triggers: /hotel|resort|suite|lobby|check in|stay|vacation|room service|airbnb/i,
  },
  {
    id: 'modern_mansion_interior',
    label: 'Modern Mansion',
    prompt: 'ultra-modern open-plan mansion interior — 6-meter high ceilings, floating glass staircase, massive white marble floors reflecting soft light, designer lighting fixtures, expensive modern art on the walls, a glimpse of an infinity pool through retractable glass walls, the atmosphere of ultimate residential wealth and architectural perfection',
    fitStyles: ['revenue', 'mrbeast'],
    fitNiches: ['lifestyle', 'finance'],
    triggers: /mansion|house|home|property|living room|villa|estate|interior design|expensive home|rich house/i,
  },
  {
  ...
  {
    id: 'medieval_castle_hall',
    label: 'Medieval Castle Hall',
    prompt: 'massive medieval stone castle hall — iron torches flickering on damp stone walls, a large heavy wooden throne in the center, high vaulted ceilings with tattered banners hanging, a large stone fireplace with a dying fire, the atmosphere of ancient power, cold stone, and historical weight',
    fitStyles: ['mrbeast', 'hunter'],
    triggers: /castle|medieval|throne|fortress|palace|king|queen|knight|dungeon|stone wall/i,
  },
  {
    id: 'forest_deep',
  ...
  // ── TECH & DIGITAL ──────────────────────────────────────────────────────────
  {
    ...
    triggers: /server|data|cloud|\bai\b|tech|computer|code|software|system|network|infrastructure|hack/i,
  },
  {
    id: 'futuristic_lab',
    ...
    triggers: /lab|research|science|experiment|test|future|technology|invention|discover|robot|\bai\b|machine/i,
  },
  {
    id: 'gaming_setup',
  ...
  {
    id: 'outer_space_station',
    label: 'Space Station / Orbit',
    prompt: 'futuristic space station interior — enormous curved observation window showing the Earth\'s blue curvature against absolute black space, high-tech consoles with glowing holographic interfaces, sterile white surfaces with recessed blue lighting, a glimpse of a docking bay, the zero-gravity atmosphere implied by floating objects, peak futuristic technology',
    fitStyles: ['mrbeast', 'revenue'],
    fitNiches: ['tech'],
    triggers: /space|moon|mars|spaceship|orbit|astronaut|galaxy|universe|alien|planet/i,
  },
  ...
  {
    id: 'prison_cell_gritty',
    label: 'Prison Cell',
    prompt: 'small gritty prison cell — cold concrete walls with scratched markings, a single rusted metal bunk bed, bars on the door with hallway light casting striped shadows across the floor, a small high window with a sliver of sky, atmosphere of isolation, regret, and limited freedom',
    fitStyles: ['caught', 'confession'],
    triggers: /prison|jail|cell|arrest|locked|sentence|inmate|criminal|custody/i,
  },
  {
    id: 'military_base_command',
    label: 'Military Command Center',
    prompt: 'top-secret military command center — wall of monitors showing satellite feeds and tactical maps, green and blue radar sweeps, rows of personnel in uniform at workstations, industrial grey equipment and headsets, the atmosphere of high-stakes operations and absolute security',
    fitStyles: ['hunter', 'caught'],
    fitNiches: ['tech'],
    triggers: /military|army|base|command|war|battle|soldier|tank|weapon|operation|mission/i,
  },
 ...
 */
function pickBest<T extends { triggers: RegExp; fitStyles?: StyleId[]; fitNiches?: Niche[] }>(
  library: T[],
  topic: string,
  style?: StyleId,
  niche?: Niche,
  ...
  const scored = library.map((item, idx) => {
    let score = 0;
    if (item.triggers.test(topic)) score += 10;
    if (style && item.fitStyles?.includes(style)) score += 6;
    if (niche && item.fitNiches?.includes(niche)) score += 4;
...
export function pickCinemaElements(
  ...
  const expression  = pickBest(EXPRESSIONS,       topic, style, niche, exprPool);
  const camera      = pickBest(CAMERA_ANGLES,     topic, style, niche, camPool);
  const textEnv     = pickBest(TEXT_ENVIRONMENTS, topic, style, niche, textPool);
  const colorGrade  = pickColorGrade(topic, style, niche);
  const background  = pickBest(BACKGROUNDS,       topic, style, niche, bgPool);
 ...
 */
function pickBestExcluding<T extends { id: string; triggers: RegExp; fitStyles?: StyleId[]; fitNiches?: Niche[] }>(
  library: T[],
  topic: string,
  style: StyleId,
  niche: Niche,
  ...
  if (eligible.length === 0) return library[0];

  const scored = eligible.map((item) => {
    let score = 0;
    if (item.triggers.test(topic)) score += 10;
    if (item.fitStyles?.includes(style)) score += 6;
    if (item.fitNiches?.includes(niche)) score += 4;
    return { item, score };
  });

  const maxScore = Math.max(...scored.map((s) => s.score));

  // If no positive score, just pick randomly from eligible
...
export function pickCinemaElementsExcluding(
  ...
  const expression = pickBestExcluding(EXPRESSIONS,       topic, style, niche, prev.expression.id);
  const camera     = pickBestExcluding(CAMERA_ANGLES,     topic, style, niche, prev.camera.id);
  const textEnv    = pickBestExcluding(TEXT_ENVIRONMENTS, topic, style, niche, prev.textEnv.id);
  const colorGrade = pickColorGradeExcluding(topic, style, niche, prev.colorGrade.id);
  const background = pickBestExcluding(BACKGROUNDS,       topic, style, niche, prev.background.id);
